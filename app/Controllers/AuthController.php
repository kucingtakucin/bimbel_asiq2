...
class AuthController extends Controller
{
    ...
    public function login()
    {
        ...
        if ($this->auth->check()) {
            if (in_groups('admin')) {
                ...
                $redirectURL = session('redirect_url') ?? route_to('guru.home');
            } elseif (in_groups('siswa')) {
                $redirectURL = session('redirect_url') ?? route_to('siswa.home');
            }
        ...
        if (in_groups('admin')) {
            $_SESSION['redirect_url'] = session('redirect_url') ?? previous_url() ?? route_to('admin.home');
        } elseif (in_groups('guru')) {
            $_SESSION['redirect_url'] = session('redirect_url') ?? previous_url() ?? route_to('guru.home');
        } elseif (in_groups('siswa')) {
            $_SESSION['redirect_url'] = session('redirect_url') ?? previous_url() ?? route_to('siswa.home');
        }
    ...
    public function attemptLogin()
    {
        ...
        return redirect()->route('admin.home')->withCookies()->with('message', lang('Auth.loginSuccess'));
    }
